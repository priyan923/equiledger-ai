...
2. **The Calculation Procrastination Trap:** Peer-to-peer group apps fail when individual users order varied items on one bill.It is a tedious mathematical chore, stalling settlement activities for weeks or months. 
...
* **Frontend Engine & Backend Routing:** `React.js` running on `Vercel Serverless Architecture`.
* **AI Vision Layer:** `Groq AI` — An open-source Optical Character Recognition subsystem executing inside the user's local web client.
